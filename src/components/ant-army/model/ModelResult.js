...
import { getModelList, getModelStatus } from '../../../util/model';
...
const ModelResult = ({ setDataEdit }) => {
    ...
    const fetch = async (_criteria) => { 
        ...
        const res = await getModelList({ ...formValue });
    ...
    const onChangeStatus = async (row) => {
        await getModelStatus({ id: row.id, status: row.status === 'Active' ? 'Inactive' : 'Active' });
